...
extern int msg_test_handler(CCustomer* WHO, void* aWHAT, void* YOU_DATA)
{
	...
	for (int i = 0; _it != _recv_arg->FMessage.FEnd; ++i, ++_it)
	{
		if (i % 255 != *_it)
		{
			...
			/*! 3) Triggering error, It will handled by #event_fail_sent_handler
			 * function of the sender.
			 *
			 */
			_recv_arg->FOccurUserError=10;
...
extern void send_messages()
{
	///Algorithm:
	for (;;
			Sleep(2000)///Send message every 2 second
			)
	{
		...
		/// 3) Corrupting random byte in the message
		
		_buf.at(rand()%_buf.size())=rand()%255;

		/*! 4) Send the message number #MESSAGE_NUMBER ver 1.2 */
