...
int initialize_library(int argc, char const*argv[])
{
	 ...
	 * number #example_of_user_protocol::E_MSG_SWAP_BYTE_TEST
	...
	CCustomer::sMGetInstance().MIWantReceivingMSG( //
			requirement_msg_info_t(PROTOCOL_NAME,
					required_header_t(msg_head_t(example_of_user_protocol::E_MSG_SWAP_BYTE_TEST, sizeof(swap_byte_order_msg_t))),
					INDITIFICATION_NAME), //
			msg_test_handler);
...
int main(int argc, char const*argv[])
{
	return example_auto_swap_endian_of_msg::main(argc, argv);
}
