...
extern  int start_child(char  const* aName)
{
	...
	const unsigned _sizeof_array=g_argc+_amount_of_additional_param+1/*NULL*/;
...
void parse_cmd(int argc, char const* argv[])
{
	...
	ValueArg<std::string> _name("n", "name",
			"id name of kernel (by default program name)", false, "",
			"string", _cmd);
...
void create_ipc_mutex()
{
	...
	if(!g_mutex_stream.MInit((uint8_t*)_p, CIPCSem::eReguredBufSize, 1,CIPCSem::E_HAS_TO_BE_NEW))
	{
...
void initialize_ipc_mutex()
{
	...
	if(!g_mutex_stream.MInit((uint8_t*)_p, CIPCSem::eReguredBufSize,  1, CIPCSem::E_HAS_EXIST))
	{
		std::cerr << "Semaphore  "<<g_sem_name<<" is exist" << std::endl;
		exit(EXIT_FAILURE);
	}
