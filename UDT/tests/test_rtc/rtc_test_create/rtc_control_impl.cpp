...
void wait_for_process_finished()
{
	...
	} while (_list.size() != 1 && (++_i < 5) && (_time + 5) > NSHARE::get_time()
			&& NSHARE::sleep(1));
...
bool test()
{
	...
	{
		LOCK_STREAM
		std::cout<<"Create RTC for RTC "<<g_rtc_name<<std::endl;
	}
...
static bool doing_tests()
{
	...
	{
		LOCK_STREAM
		std::cerr << std::endl << std::endl << std::endl << "Test 1 failed" << std::endl;
		getchar();
		return false;
	}
	else
...
static bool test_integer_timers()
{
	...
	IRtc * g_rtc = CCustomer::sMGetInstance().MGetOrCreateRTC(g_rtc_name,
			eRTC_MODELING);
	CHECK_NOTNULL(g_rtc);
	{
		LOCK_STREAM
		std::cout<<"RTC created "<<g_rtc_name<<std::endl;
	}
	bool _is = true;
	...
	NSHARE::sleep(1);
	_is = _is && g_rtc->MGetAmountOfJoined() == (g_child_pid.size() + 1);
	...
	if (_is)
	{
		...
		auto _less_eq_then =
				[& _precision](decltype(_precision) aVal,decltype(_precision) aMin)
		...
		for (unsigned i = 0; i < g_number_of_time && _is; ++i)
		{

			{
				...
				if (_is)
				{
					...
					decltype(g_next_times)::value_type const _min =
							g_next_times.front();
				...
				else
				{
					_is = false;
					LOCK_STREAM
					std::cerr << "No data:" << g_rtc->MGetCurrentTime()
							<< std::endl;
				}
			}
			if (_is)
			{
				...
				if (_last_time <= i || i == (g_number_of_time - 1))
				{
					...
					LOCK_STREAM
					std::cout << "Time:" << g_rtc->MGetCurrentTime() << " i=" << i
							<< std::endl;
				}
			}
		}
	...
	CCustomer::sMGetInstance().MRemoveRTC(g_rtc_name);
	return _is;
}
...
int msg_next_time_handler(CCustomer* WHO, void* aWHAT, void* YOU_DATA)
{
	...
	const msg_next_time_t* _control =
			(const msg_next_time_t*) _recv_arg->FMessage.FBegin;
