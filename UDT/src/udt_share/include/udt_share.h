...
extern UDT_SHARE_EXPORT int init_id(char const *aName, eProgramType aType,
		NSHARE::version_t const& aVer);

extern UDT_SHARE_EXPORT bool fill_dg_head(void* aWhat, size_t aFullSize,
		const program_id_t& aFrom, bool aIsNeedCrc = false);
...
extern UDT_SHARE_EXPORT bool serialize(NSHARE::CBuffer* aTo,
		const user_data_info_t& aWhat, size_t aSize);
extern UDT_SHARE_EXPORT size_t serialize(NSHARE::CBuffer* aTo,
		const user_data_t& aWhat, bool aIsNeedCrc = false);
...
extern size_t UDT_SHARE_EXPORT fill_header(NSHARE::CBuffer::pointer aTo,
		user_data_info_t const& aData,
		NSHARE::CBuffer::offset_pointer_t aOffset);

template<class aKdTypeY, class T>
inline size_t serialize(NSHARE::CBuffer* _buf, const T& aWhat,
		const routing_t& aRoute, error_info_t const&aError)
{
	...
	NSHARE::CBuffer::value_type* const _p_begin =
			(NSHARE::CBuffer::value_type*) _buf->ptr()
					+ _begin;
	...
	for (unsigned i = 0; i < _str_size; ++i)
		*_p++ = static_cast<NSHARE::CBuffer::value_type> (_text[i].FVal);
	*_p++ = '\0';
...
inline void deserialize_route_impl(NSHARE::CConfig const& aConf,
		routing_t* aRoute)
{
	if (aRoute)
	{
...
inline void deserialize_error_impl(NSHARE::CConfig const& aConf,
		error_info_t* aError)
{
	if (aError)
	{
...
template<class Tto>
inline Tto deserialize_impl(NSHARE::CConfig const& aConf, routing_t* aRoute,
		error_info_t *aError)
{
...
template<>
inline program_id_t deserialize_impl<program_id_t>(NSHARE::CConfig const& aConf,
		routing_t* aRoute, error_info_t *aError)
{
...
inline Tto deserialize(aKdTypeY const* aP, routing_t* aRoute,
		error_info_t *aError)
{
	...
	return deserialize_impl<Tto>(_conf,
			aP->MIs(aKdTypeY::E_ROUTE_OF_MSG) ? aRoute : NULL,
			aP->MIs(aKdTypeY::E_ERROR_OF_MSG) ? aError : NULL);
}
