...
NSHARE::CText const CControlByTCPRegister::NAME = "tcp_io_manager";
...
void CControlByTCP::MFill<dg_info2_t>(data_t* aTo)
{
	...
	programm_id_t  const _id = CCustomer::sMGetInstance().MGetID();
...
void CControlByTCP::MProcess(requiest_info2_t const* aP, void*)
{
...

	{
		...
		{
			programm_id_t _id(
					deserialize<requiest_info2_t, programm_id_t>(aP, NULL,
							NULL));
			FKernelId.MSet(_id);
...
#if !defined(TCP_IO_MANAGER_STATIC)
static NSHARE::factory_registry_t g_factory;
extern "C" TCP_IO_MANAGER_EXPORT NSHARE::factory_registry_t* get_factory_registry()
{
	if (g_factory.empty())
	{
