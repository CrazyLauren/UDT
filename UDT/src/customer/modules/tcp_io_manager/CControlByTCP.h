...
#if  defined( _WIN32 ) && defined(_MSC_VER) && !defined( TCP_IO_MANAGER_STATIC)
#   ifdef TCP_IO_MANAGER_EXPORTS
#       define  TCP_IO_MANAGER_EXPORT __declspec(dllexport)
#   else
#       define TCP_IO_MANAGER_EXPORT __declspec(dllimport)
#   endif
#else
#       define TCP_IO_MANAGER_EXPORT
#endif
...
namespace NUDT
{
class TCP_IO_MANAGER_EXPORT CControlByTCP: public IIOConsumer, NSHARE::CAsyncSocket
{
public:
...
private:
	...
	ICustomer* FCustomer;
	ILocalChannel* FMain;
	NSHARE::smart_field_t<programm_id_t> FKernelId;
