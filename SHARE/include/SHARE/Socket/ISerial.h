...
public:
	...
	static SHARED_PTR<CNotation> sCreateNotation(CText const& aType);
...
struct SHARE_EXPORT port_settings_t
{
	...
	port_settings_t(NSHARE::CConfig const& aConf)
	{
		...
		{
			SHARED_PTR<CNotation> _not(CNotation::sCreateNotation(_name));
			if(_not.get())
				_not->MSetting(*this);
		}
	}
...
inline SHARED_PTR<CNotation> CNotation::sCreateNotation(CText const& aType)
{
	CNotation *_ptr=NULL;
	if (aType == "8N1")
		_ptr= new C8N1();
	...
	return SHARED_PTR<CNotation>(_ptr);
};
