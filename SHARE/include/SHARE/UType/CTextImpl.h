...
{
	return const_iterator(ptr_const());
}
...
{
	return const_iterator(&ptr_const()[FCodePointLength]);
}
...
struct equal_to<NSHARE::CText>: public binary_function<NSHARE::CText, NSHARE::CText, bool>
{
	bool operator()(const NSHARE::CText& a, const NSHARE::CText& b) const
	{
		...
		if (la == lb)
		{
			if (la!=0 && a.ptr_const()!=b.ptr_const())
				return (memcmp(a.ptr_const(), b.ptr_const(), la * sizeof(NSHARE::utf32)) == 0);
			else
				return true;
		}
		return false;
