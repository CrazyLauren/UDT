...
DEFINE_FLAG(int, minloglevel, FATAL)
;
...
log4cplus::Logger& _log4cplus_impl::get_console_logger(eLavel _level)
{
	...
	_logger.setLogLevel(convert(INFO));
...
void _log4cplus_impl::init_log4cplus(char *argv)
{
	if (is_inited())
		return;
	is_inited() = true;
...
std::list<_log4cplus_impl::module_info_t> _log4cplus_impl::init_modules()
{
	MASSERT_1(_log4cplus_impl::is_inited());
...
std::list<_log4cplus_impl::module_info_t>& _log4cplus_impl::get_modules()
{
...
bool _log4cplus_impl::fnmatch(const char* pattern, size_t patt_len,
		const char* str, size_t str_len) //from google
{
	size_t p = 0;
	size_t s = 0;
	while (1)
...
bool _log4cplus_impl::is_file_level(int** aVal, const char* aFile)
{
	MASSERT_1(_log4cplus_impl::is_inited());
...
int _log4cplus_impl::get_file_level(const char* aFile)
{
...
bool _log4cplus_impl::is_v_log_on(const char* aFile, int verboselevel)
{
	if (!_log4cplus_impl::is_inited())
		return FLAGS_v>= verboselevel;
		else
		return get_file_level(aFile) >= verboselevel;
	}
int _log4cplus_impl::vlog_uniitialized = 50000;
bool _log4cplus_impl::init_gnu_vlog(int** aLocal, int* aDefault,
...
{
	_log4cplus_impl::init_log4cplus(argv[0]);
