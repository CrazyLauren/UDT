...
//                  "%02x-%02x-"
//                  "%02x-%02x-"
//                  "%02x-%02x", _this[7], _this[6], _this[5], _this[4], _this[3], _this[2], _this[1], _this[0]);
    _text<<std::showbase<< std::setfill('0')<< std::setw(2)<<std::hex ;
    for (int i=sizeof(FVal); --i>0;)
    {
    ...
    _text<<std::showbase<< std::setfill('0')<< std::setw(2)<<std::hex ;
    for (unsigned i=0; i<sizeof(FVal); i++)
    {
        _text<<(unsigned)_this[i];
        if(i!=(sizeof(FVal)-1))
            _text<<'-';
...
bool uuid_t::MFromString(CText const &aText)
{
    ...
    int count = sscanf(aText.c_str(), "%*c%*c%02x%*c" //
                                      "%*c%*c%02x%*c"//
                                      "%*c%*c%02x%*c"//
                                      "%*c%*c%02x%*c"//
                                      "%*c%*c%02x%*c"//
